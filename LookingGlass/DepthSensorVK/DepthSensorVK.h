...
public:
	...
	virtual void CreateTexture() override {
		...
		AnimatedTextures.emplace_back().Create(Device, CurrentPhysicalDeviceMemoryProperties, VK_FORMAT_R8_UNORM, VkExtent3D({ .width = Frame.Header.Cols, .height = Frame.Header.Rows, .depth = 1 }));
	}
	...
	virtual void DrawFrame(const UINT i) override {
		AnimatedTextures[0].UpdateStagingBuffer(Device, Frame.Header.Cols * Frame.Header.Rows * 1, std::data(Frame.Payload));
	}
